...
{
    public class OtherController : NetworkBehaviour, ICreatureController
    {
        ...
        private Transform FindCharacter()
        {
            ...
            switch (gameObject.tag)
            {
                case "Enemy":
                    {
                        var chars = GameObject.FindGameObjectsWithTag("Character").ToList();
                        objs = chars;
                        var allies = GameObject.FindGameObjectsWithTag("Ally").ToList();
                        objs.AddRange(allies);
                        break;
                    }
                case "Ally":
                    {
                        var bosses = GameObject.FindGameObjectsWithTag("Boss").ToList();
                        objs = bosses;
            ...
            return RandomTarget(objs);
        }

        private Transform RandomTarget(List<GameObject> objs)
        {
            if (objs.Count == 0) return null;
            if (objs.Count == 1) return objs[0].transform;

            var rand = Random.Range(0, objs.Count);
